...
The full click data and number at risk tables can be loaded as shown below.
Now let's visualize the data. Use the cmprsk package to plot the cumulative incidence functions of the two events in the two groups. Then plot the standard Kaplan-Meier curve of PFS, where death and progression are combined into a composite event:
...
```{r reconstruct KM, eval=TRUE, results='show', warning=FALSE}
...
And here we can reconstruct and plot the reconstructed cumulative incidence curves:

```{r reconstruct CIC, eval=TRUE, results='show', warning=FALSE}
# load cic data
